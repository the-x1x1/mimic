...
**On macOS (compiled, never run)** the values are sealed with a key kept in the user's login Keychain: one generic password, service "Mimic", account "credentials key", made from the system's random generator the first time a value is saved (`crates/keychain`, `login_keychain::LoginKeychain`). Each value is sealed with XChaCha20-Poly1305 under a fresh random 24-byte nonce, bound to Mimic's own label, and written hex-encoded like a DPAPI value, reported as `keychain`. A copy of the file cannot be opened without that Keychain item, and a value altered in the file does not open at all. A program running as the user can ask for the item, as it can for the browser's saved passwords — macOS may ask the user first. The sealing is tested wherever the tests run, with a key the test holds (`mimic-keychain`'s own tests, and `secrets::tests::with_the_keychain_a_value_is_sealed_and_opens_only_with_its_key`); the Keychain calls are checked with `cargo check --target aarch64-apple-darwin` and have never been made. Values a development build on macOS wrote unsealed before this are reported as locked, not read.
...
- **What the screen says is computed** by the store: `protection` is what this build does (`account` with DPAPI, `keychain` with the login Keychain, `file` with neither), never what was meant to happen.
- **Other platforms.** Development builds that are neither Windows nor macOS write the values unsealed to an owner-only (`0600`) file and report `file`.
